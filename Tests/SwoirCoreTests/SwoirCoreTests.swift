...
final class SwoirCoreTests: XCTestCase {
    ...
    func testErrorCases() throws {
        ...
        XCTAssertThrowsError(try MockSwoirBackend.verify(proof: proofEmptyProof, proof_type: "plonk", num_points: 0)) { error in
            XCTAssertEqual(error as? SwoirBackendError, .emptyProofData)
        }
        XCTAssertThrowsError(try MockSwoirBackend.verify(proof: proofEmptyVKey, proof_type: "plonk", num_points: 0)) { error in
